...
public class Viz extends JPanel {
	...
	private static int TEXT_COLUMN_SIZE = 160; // size of the left column of
												// labels (in pixels)
	private static int PIXELS_PER_HOUR = 40;
	private static int HEIGHT_OF_WHISKER = 16; // height of one whisker on the
												// diagram (in pixels)
	private static int SPACE_BETWEEN_WHISKERS = 5; // (in pixels)
	private static int LABEL_X_POSITION = 20; // size of the left margin before
	...
	public static void makeScale(Graphics g, int whichLevel) {
		...
		g.drawLine(pxlCoord(0), yCoord, pxlCoord(1440), yCoord);
		g.drawLine(pxlCoord(0), yCoordMaker(0), pxlCoord(0), yCoord);
		int pxl = 0;
		for (int i = 0; i < 25; i++) {
			pxl = pxlCoord(i * 60);
